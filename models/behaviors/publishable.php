...
class PublishableBehavior extends ModelBehavior {
   ...
   var $_built = array();
	...
	function isPublished(&$model, $id = '') {
	   ...
	   if(empty($page[$model->alias][$this->__settings[$model->alias]['start_column']]) 
          || strtotime($page[$model->alias][$this->__settings[$model->alias]['start_column']]) > time() 
          || (!empty($page[$model->alias][$this->__settings[$model->alias]['end_column']]) && strtotime($page[$model->alias][$this->__settings[$model->alias]['end_column']]) < time())) {
          return false;
      }
      else return true;
	}
