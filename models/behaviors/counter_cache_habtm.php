...
class CounterCacheHabtmBehavior extends ModelBehavior {
	...
	function findForeignIDs(&$model) {
		foreach ($model->hasAndBelongsToMany as $assocKey => $assocData) {
			...
			if (!empty($assocModel) && $assocModel->hasField($field)) {
		...
		foreach ($model->hasAndBelongsToMany as $assocKey => $assocData)
			if (isset($this->foreignTableIDs[$assocData['className']])
				...
				if ($assocModel->hasField($field)) {
					...
					// 'counterScope'による条件指定を追加する
					$conditions[] = array($assocData['associationForeignKey'] => $this->foreignTableIDs[$assocData['className']]);
					if (method_exists($model ,'habtmCounterScope')) {
						$conditions[] = $model->habtmCounterScope($field);
					}
					...
					foreach ($counts as $assocId => $count) {
						...
						$results[$assocModel->name] = $assocModel->saveAll($saveArr, array(
							'validate' => false,
	...
	function afterSave(&$model, $created) {
		...
		foreach ($model->hasAndBelongsToMany as $assocKey => $assocData) {
			...
			if ($model->hasField($field)) {
				...
				$conditions[] = array($assocData['foreignKey'] => $model->id);
				$assocModel =& $model->{$assocData['className']};
				if (method_exists($assocModel ,'habtmCounterScope')) {
					$conditions[] = $assocModel->habtmCounterScope($field);
				}
				$count = $joinModel->field(
					($assocData['unique'] ? 'COUNT(*)' : 'COUNT(DISTINCT '.$assocData['associationForeignKey'].')').' AS count',
					$conditions
				);
				$results[$assocKey] = $model->saveField($field, $count, array(
					'validate' => false,
					'callbacks' => false
				));
			}
